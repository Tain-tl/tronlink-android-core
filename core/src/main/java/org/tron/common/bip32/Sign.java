...
import static org.tron.common.bip32.Assertions.verifyPrecondition;
...
//        return MESSAGE_TRXPREFIX.concat(String.valueOf(32)).getBytes(StandardCharsets.UTF_8);
    ...
    static byte[] getEthereumMessageHashV2(byte[] message) {
        byte[] prefix = getEthereumMessagePrefix(message.length);
        return getEthereumMessageHash(message, prefix);
    }

    public static byte[] getPrefixedMessageHash(byte[] message) {
        return getEthereumMessageHash(message);
    }
    public static byte[] getPrefixedMessageHashV2(byte[] message) {
        return getEthereumMessageHashV2(message);
    }
    ...
    public static SignatureData signPrefixedMessageV2(byte[] message, ECKeyPair keyPair) {
        return signMessage(getEthereumMessageHashV2(message), keyPair, false);
    }

    public static SignatureData signPrefixedMessage(byte[] message, ECKeyPair keyPair, boolean needToHash) {
        return signMessage(getEthereumMessageHash(message), keyPair, needToHash);
    }
